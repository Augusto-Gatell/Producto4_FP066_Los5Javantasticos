...
function draganddrop() {
  ...
  $( ".column" ).sortable({
    ...
    start: function(event, ui) {
      // Personaliza la apariencia del elemento de arrastre aquí
      // ui.helper es el elemento que se está arrastrando
      ui.helper.addClass("dragging");
    },

    stop: function(event, ui) {
      // Personaliza el comportamiento después del arrastre aquí
      // ui.item es el elemento que fue arrastrado
...
function drawModalTask(index, yearweek = '', dayofweek = '') {
  ...
  var field_file = '<label for="file" class="form-label"><strong>Archivo adjunto</strong></label>';
  if( task.file ) {
    field_file += '<div><a href="'+task.file+'" id="file-link" target="_blank">Ver archivo</a></div>';
    field_file += '<div class="form-check"><input class="form-check-input" type="checkbox" value="1" id="file-delete" name="file-delete"><label class="form-check-label" for="file-delete">Eliminar archivo</label></div>';
  }
  field_file += '<input type="file" id="file" name="file"  class="form-control" />';

  // preparamos html del formulario con los campos
  var formHtml = '<div class="modal" tabindex="-1" id="modal-tarea"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">'+title+' tarea</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><form id="tarea-form" class="needs-validation" enctype="multipart/form-data"><div class="modal-body">'+fields_hidden+'<div class="mb-3">'+field_name+'</div><div class="mb-3">'+field_description+'</div><div class="mb-3">'+field_color+'</div><div class="mb-3 row"><div class="col-6">'+field_time_start+'</div><div class="col-6">'+field_time_end+'</div></div><div class="mb-3">'+field_priority+'</div><div class="mb-3">'+field_finished+'</div><div class="mb-3">'+field_file+'</div></div><div class="modal-footer"><button id="modal-tarea-button" type="submit" class="btn btn-primary">Guardar</button></div></form></div></div></div>';
  ...
  form.addEventListener('submit', function (event) {
    ...
    $.each($('#tarea-form').serializeArray(), function(i, field) {
      if (['finished','priority','file-delete'].includes(field.name)) {
          values[field.name] = parseInt(field.value);
      } else {
            values[field.name] = field.value;
        }
    });
    if( values['file-delete'] ) {
      values['file'] = '';
    } else {
      values['file'] = task.file;
      if( typeof document.getElementById('file').files[0] != 'undefined' ) {
        values['file'] = `files/${document.getElementById('file').files[0].name}`;
      }
    }

    var newIndice = values['id'];
    
    // Sube el archivo utilizando una solicitud fetch independiente
    var fileData = new FormData();
    fileData.append('file', document.getElementById('file').files[0]);
    fetch('http://localhost:3000/tasks/upload', {
      method: 'POST',
      body: fileData,
...
const subscriptionUrl = "ws://localhost:3000/graphql";

const subscribeToTaskUpdates = () => {
  console.log("INICIO: " + Date());

  // Crear un cliente de GraphQL-WS
  ...
  client.subscribe(
    ...
    {
      // Se llama para cada evento de actualización
      next: (data) => {
        const taskId = data.data.taskUpdated.id;
      ...
      // Se llama cuando la suscripción se cierra
      complete: () => {
        console.log("Connection closed. Retrying in 5 seconds...");
        setTimeout(subscribeToTaskUpdates, 5000);
      },
    }
  );
};
